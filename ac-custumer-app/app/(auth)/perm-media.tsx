...
export default function PermMediaScreen() {
  ...
  // Check real media library permission status on mount
  useEffect(() => {
    const checkPermission = async () => {
      try {
        ...
        console.log('Error checking media library permission:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleRequest = async () => {
    try {
      ...
      if (status === 'granted') {
        setIsAllowed(true);
        Alert.alert("Granted", "Media Library access has been successfully granted.");
      } else {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <Image
        source={require('../../assets/permissions_media_bg.png')}
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: colors.textSecondary }]}>STEP 3 OF 4</Text>
        <View style={{ width: 40 }} />
      </View>

      <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
        <View style={styles.contentCard}>
          <Text style={[styles.brandText, { color: colors.primary }]}>MEDIA LIBRARY</Text>
          <Text style={[styles.title, { color: colors.text }]}>Enable Media Access</Text>
          <View style={[styles.headerDivider, { backgroundColor: colors.primary + '20' }]} />
          
          <Text style={[styles.desc, { color: colors.textSecondary }]}>
            This allows you to choose and upload pre-saved images or troubleshooting videos of your AC units directly from your photo library to simplify maintenance diagnostics.
          </Text>
          ...
          {!loading && isAllowed ? (
            ...
            >
              <Text style={styles.highlightBadgeText}>✓ Media Access Enabled</Text>
            </MotiView>
          ) : (
      ...
      <View style={styles.footer}>
        {!loading && isAllowed ? (
          <PrimaryButton 
            ...
            onPress={() => router.push('/(auth)/perm-microphone')} 
          />
        ) : (
          <PrimaryButton 
