...
export default function PermCameraScreen() {
  ...
  // Check real camera permission status on mount
  useEffect(() => {
  ...
  const handleRequest = async () => {
    try {
      ...
      if (status === 'granted') {
        setIsAllowed(true);
        Alert.alert("Granted", "Camera access has been successfully granted.");
      } else {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <Image
        source={require('../../assets/permissions_camera_bg.png')}
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: colors.textSecondary }]}>STEP 2 OF 4</Text>
        <View style={{ width: 40 }} />
      </View>

      <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
        <View style={styles.contentCard}>
          <Text style={[styles.brandText, { color: colors.primary }]}>CAMERA ACCESS</Text>
          <Text style={[styles.title, { color: colors.text }]}>Enable Camera Access</Text>
          <View style={[styles.headerDivider, { backgroundColor: colors.primary + '20' }]} />
          
          <Text style={[styles.desc, { color: colors.textSecondary }]}>
            This lets you snap and upload clear photos of your AC unit, visual diagnostic labels, or leakage issues to help our techs prepare the correct replacement parts before arrival.
          </Text>
          ...
          {!loading && isAllowed ? (
            ...
            >
              <Text style={styles.highlightBadgeText}>✓ Camera Access Enabled</Text>
            </MotiView>
          ) : (
      ...
      <View style={styles.footer}>
        {!loading && isAllowed ? (
          <PrimaryButton 
            ...
            onPress={() => router.push('/(auth)/perm-media')} 
          />
        ) : (
          <PrimaryButton 
